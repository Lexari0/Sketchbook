...
if [ -d "/media/$DRIVE/content" ]; then
    PORT = `/usr/bin/cat $DIR/config.yaml | /usr/bin/grep port: | /usr/bin/rev | /usr/bin/cut -d' ' -f1 | /usr/bin/rev`
    KEY = `/usr/bin/cat $DIR/config.yaml | /usr/bin/grep key: | /usr/bin/rev | /usr/bin/cut -d' ' -f1 | /usr/bin/rev`
