...
module.exports = {
    register_endpoints: endpoints => {
        ...
        endpoints[/^\/api\/gallery\/item\/[0-9]+$/] = async (req, res) => {
            ...
            if (item !== undefined)
            {
                api.sendResponse(res, 200, {error: "", item, visibility: await gallery.isItemCensoredWithCookies(gallery_item_id, {
            ...
            try
            {
                await new Promise((resolve, reject) => {
                    ...
                    form.parse(req, async (err, fields, form) => {
                        ...
                        try
                        {
                            const old_file_is_missing = await gallery.isItemMissing(gallery_item_id);
                            log.message("api", `old_file_is_missing: ${old_file_is_missing}`); // temp
                            if (old_file_is_missing === false)
                            {
